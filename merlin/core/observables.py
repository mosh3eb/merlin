...
def parse_observable(expr: str, n_modes: Optional[int] = None) -> Union[PauliObservable, CompositeObservable, NumberOperator]:
    """
    Parse observable expression string.

    Args:
        expr: Expression like "ZZI", "0.5*ZZI + 0.5*IZZ", or "n_0" for number operator
        n_modes: Optional number of modes for validation

    Returns:
        PauliObservable, NumberOperator, or CompositeObservable
    """
    expr = expr.strip()
