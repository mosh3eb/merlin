...
- ``return_object``: Choose to return a typed object fo the forward output depending  on the ``measurement_strategy``. Take a look at :doc:`../api_reference/api/merlin.algorithms.layer` for more details about the return types.
