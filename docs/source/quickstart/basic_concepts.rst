...
.. code-block:: python
    ...
    quantum_layer = ML.QuantumLayer(
        ...
        return_object=False                             # Choose wether or not to return a typed object after a forward call depending on the measurement strategy.
    )
...
- ``return_object``: Choose to returned a typed object fo the forward output depending  on the ``measurement_strategy``. Take a look at :doc:`../api_reference/api/merlin.algorithms.layer` for more details about the return types.
