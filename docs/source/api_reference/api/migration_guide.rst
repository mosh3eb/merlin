...
.. deprecated:: 0.4
   ...
   ``computation_space`` instead. See this migration section for the mapping.
   
The ``no_bunching`` flag is deprecated. When you need to control how Fock states are
truncated or encoded, use ``computation_space`` on ``QuantumLayer`` instead. Map the
legacy intent as follows:
